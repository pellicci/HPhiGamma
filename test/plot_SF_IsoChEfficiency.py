...
# Input
# --------------------------------------------------------------------------------------
inputFile    = ROOT.TFile("histos/IsoEfficiency/histos_IsoChEfficiencySimFit_"+ETA+".root")
inputFilePhi = ROOT.TFile("histos/IsoEfficiency/histos_IsoChEfficiencySimFit_Phi_"+ETA+".root")
inputFileRho = ROOT.TFile("histos/IsoEfficiency/histos_IsoChEfficiencySimFit_Rho_"+ETA+".root")
inputFileK0s = ROOT.TFile("histos/IsoEfficiency/histos_IsoChEfficiencySimFit_K0s_"+ETA+".root")
...
print("Number of bins in h_k0s     : ", h_k0s.GetNbinsX())
...
def create_ratio_plot(h_data, h_mc, h_phi, h_rho, h_k0s):
    ...
    h_data.GetYaxis().SetTitle("IsoCh Efficiency")
    ...
    # Create legend
    legend = ROOT.TLegend(0.2, 0.1, 0.4, 0.42)
    legend.SetBorderSize(0)
    legend.SetFillColor(0)
    legend.SetTextSize(0.04)
    legend.AddEntry("","IsoCh efficiency, "+ETA)
    ...
    h_data.GetYaxis().SetRangeUser(0.8, 1.05)
    ...
    h_ratio.SetMinimum(0.92)
    h_ratio.SetMaximum(1.08)
    h_ratio.SetTitle("")
    ...
    canvas.Update()
    canvas.SaveAs("~/cernbox/www/latest_production/IsoChEfficiency_Fit_"+ETA+"_latest_production/IsoChEfficiencySF_"+ETA+".png")
    canvas.SaveAs("~/cernbox/www/latest_production/IsoChEfficiency_Fit_"+ETA+"_latest_production/IsoChEfficiencySF_"+ETA+".pdf")


create_ratio_plot(h_data, h_mc, h_phi, h_rho, h_k0s)

fOut = ROOT.TFile("scale_factors/IsoChEfficiencySF_"+ETA+".root","RECREATE")
fOut.cd()

...
for SAMPLE in ["Data", "MC"]:
	...
	for binIndex in binList:
		...
		# I n p u t   f i l e s
		# -------------------------------------------------------------
		fileInput = ROOT.TFile("histos/IsoEfficiency/histos_IsoChEfficiency_"+SAMPLE+"_"+ETA+".root")
		fileInput.cd()
		...
		# Create observables
		mass = ROOT.RooRealVar("mumuMass","mumu invariant mass",75.,105.)
		...
		if binIndex == 0 or 2 or 3:
			sigma_central , sigma_min , sigma_max  = 0.8,0.5,2.
		...
		canvas.SaveAs("~/cernbox/www/latest_production/IsoChEfficiency_Fit_"+ETA+"_latest_production/simultaneous_fit_bin_"+str(binIndex)+"_"+SAMPLE+".png")
		canvas.SaveAs("~/cernbox/www/latest_production/IsoChEfficiency_Fit_"+ETA+"_latest_production/simultaneous_fit_bin_"+str(binIndex)+"_"+SAMPLE+".pdf")


fOut = ROOT.TFile("histos/IsoEfficiency/histos_IsoChEfficiencySimFit_"+ETA+".root","RECREATE")
fOut.cd()

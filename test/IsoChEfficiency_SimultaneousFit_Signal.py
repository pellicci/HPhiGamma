...
h_efficiency_Signal = ROOT.TH1F("h_efficiency_Signal","eff MC",len(pT_list) - 1, array('d', pT_list))
...
for binIndex in binList:
	...
	# I n p u t   f i l e s
	# -------------------------------------------------------------
	fileInput = ROOT.TFile("histos/IsoEfficiency/histos_IsoChEfficiency_"+CHANNEL+"_"+ETA+".root")
	fileInput.cd()
	...
	# Create observables
	if CHANNEL == "Phi":
		mass = ROOT.RooRealVar("mesonMass","meson invariant mass",1.,1.05)
	if CHANNEL == "Rho":
		mass = ROOT.RooRealVar("mesonMass","meson invariant mass",0.5,1.)		
	if CHANNEL == "K0s":
		mass = ROOT.RooRealVar("mesonMass","meson invariant mass",0.8,1.)	

	# Construct signal pdf (Double-sided Crystal Ball)
	if CHANNEL == "Phi":
		mean_central  ,  mean_min ,  mean_max  = 1.02,1., 1.05
		sigma_central , sigma_min , sigma_max  = 0.0012,0.0001,0.01
	if CHANNEL == "Rho":
		mean_central  ,  mean_min ,  mean_max  = 0.77,0.5, 1.
		sigma_central , sigma_min , sigma_max  = 0.018,0.001,0.1
	if CHANNEL == "K0s":
		mean_central  ,  mean_min ,  mean_max  = 0.892,0.8, 1.
		sigma_central , sigma_min , sigma_max  = 0.005,0.0005,0.03
	...
	if binIndex == 0 or 2 or 3:
		a1_central, a1_min, a1_max = 0.94,-0.2,1.5
		a2_central, a2_min, a2_max = 0.07,-0.5,0.5
		a3_central, a3_min, a3_max = 0.3,-2.,2.
	else:
		a1_central, a1_min, a1_max = 0.94,-0.1,1.5
		a2_central, a2_min, a2_max = 0.07,-0.2,0.2
		a3_central, a3_min, a3_max = 0.3,-2.,2.
	...
	#nbkgFail   = ROOT.RooRealVar("nbkgFail", "background yield", 0., 0., 100.)
	#nbkgPass   = ROOT.RooRealVar("nbkgPass", "background yield", 0., 0., 100.)

	nbkgFail   = ROOT.RooRealVar("nbkgFail", "background yield", nEntries/8, 0., nEntries/2)
	nbkgPass   = ROOT.RooRealVar("nbkgPass", "background yield", nEntries/8, 0., nEntries/2)


	efficiency = ROOT.RooRealVar("efficiency", "efficiency", 0.8, 0., 1.)
	...
	frame1.GetXaxis().SetTitle("m_{meson} [Gev]")
	...
	frame2.GetXaxis().SetTitle("m_{meson} [Gev]")
	...
	h_efficiency_Signal.SetBinContent(binIndex+1, eff)
	h_efficiency_Signal.SetBinError(binIndex+1, eff_err)

	canvas.SaveAs("~/cernbox/www/latest_production/IsoChEfficiency_Fit_"+ETA+"_latest_production/simultaneous_fit_bin_"+str(binIndex)+"_"+CHANNEL+".png")
	canvas.SaveAs("~/cernbox/www/latest_production/IsoChEfficiency_Fit_"+ETA+"_latest_production/simultaneous_fit_bin_"+str(binIndex)+"_"+CHANNEL+".pdf")


fOut = ROOT.TFile("histos/IsoEfficiency/histos_IsoChEfficiencySimFit_"+CHANNEL+"_"+ETA+".root","RECREATE")
fOut.cd()

h_efficiency_Signal.Write()

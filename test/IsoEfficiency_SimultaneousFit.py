...
for SAMPLE in ["Data", "MC"]:
	...
	for binIndex in binList:
		...
		# I n p u t   f i l e s
		# -------------------------------------------------------------
		fileInput = ROOT.TFile("histos/IsoEfficiency/histos_IsoEfficiency_"+SAMPLE+".root")
		fileInput.cd()
		...
		# Create observables
		mass = ROOT.RooRealVar("mumuMass","mumu invariant mass",70.,110.)

		# Construct signal pdf (Double-sided Crystal Ball)
		mean_central  ,  mean_min ,  mean_max  = 91., 89., 93.
		sigma_central , sigma_min , sigma_max  = 1.,0.5,10.
		...
		#in case of Voigtian
		width_central , width_min , width_max  = 1.,0.5,10.
		...
		nbkgFail   = ROOT.RooRealVar("nbkgFail", "background yield", 500., 0., 10000.)
		nbkgPass = ROOT.RooRealVar("nbkgPass", "background yield", 500., 0., 100000.)
		efficiency = ROOT.RooRealVar("efficiency", "efficiency", 0.6, 0., 1.)
		...
		jointData = ROOT.RooDataSet("jointData", "joint data", ROOT.RooArgSet(mass), ROOT.RooFit.Index(sample), ROOT.RooFit.Import("Fail", data1), ROOT.RooFit.Import("Pass", data2))
		...
		canvas.SaveAs("~/cernbox/www/latest_production/IsoEfficiency_Fit_latest_production/simultaneous_fit_bin_"+str(binIndex)+"_"+SAMPLE+".png")
		canvas.SaveAs("~/cernbox/www/latest_production/IsoEfficiency_Fit_latest_production/simultaneous_fit_bin_"+str(binIndex)+"_"+SAMPLE+".pdf")


fOut = ROOT.TFile("histos/IsoEfficiency/histos_IsoEfficiencySimFit.root","RECREATE")
fOut.cd()

...
#Import dataset ---------------------------------------------------------------------------
data = ROOT.RooDataSet("data","data",ROOT.RooArgSet(mass),ROOT.RooFit.Import(mytree))
...
#Signal PDF
if isDoubleCB:
	workspaceSignal.var("dCB_aL").setConstant(1) #Set constant these parameters of the signal PDF, assuming the MC well describes the signal shape
...
B_R_blind = ROOT.RooUnblindOffset("B_R_blind","B_R_blind","aSeedString",0.000001,B_R) #generate an unkown offset to be applied to the BR when performing the blind analysis
...
#Plots -------------------------------------------------------------------------------------------------------------------
xframe = mass.frame(100.,150.,30) #min, max, nBins
xframe.SetTitle(" ")
xframe.SetTitleOffset(1.4,"y")
#xframe.SetMaximum(90)

#Exclude the control regions
#data_reduced = data.reduce("mass_KKg < 120. || mass_KKg > 130.")
data_reduced = copy.deepcopy(data).reduce("mass_KKg < 120. || mass_KKg > 130.")
...
#Write to file -------------------------------------------------------------------------------------------------------------------
fOutput = ROOT.TFile("workspaces/ws_data_blind_offset.root","RECREATE")
fOutput.cd()

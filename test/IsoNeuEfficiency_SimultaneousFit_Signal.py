...
for binIndex in binList:
	...
	# I n p u t   f i l e s
	# -------------------------------------------------------------
	fileInput = ROOT.TFile("histos/IsoEfficiency/histos_IsoNeuEfficiency_"+CHANNEL+"_"+ETA+".root")
	fileInput.cd()
	...
	if CHANNEL == "Rho":
		...
		sigma_central , sigma_min , sigma_max  = 0.012,0.001,0.1
	if CHANNEL == "K0s":
		mean_central  ,  mean_min ,  mean_max  = 0.892,0.8, 1.
		sigma_central , sigma_min , sigma_max  = 0.005,0.0005,0.05
	...
	if CHANNEL == "Rho": signalPDF = gaussian
	...
	nbkgFail   = ROOT.RooRealVar("nbkgFail", "background yield", nEntries/4, 0., nEntries)
	nbkgPass   = ROOT.RooRealVar("nbkgPass", "background yield", nEntries/10, 0., nEntries)
	...
	canvas.SaveAs("~/cernbox/www/latest_production/IsoNeuEfficiency_Fit_"+ETA+"_latest_production/simultaneous_fit_bin_"+str(binIndex)+"_"+CHANNEL+".png")
	canvas.SaveAs("~/cernbox/www/latest_production/IsoNeuEfficiency_Fit_"+ETA+"_latest_production/simultaneous_fit_bin_"+str(binIndex)+"_"+CHANNEL+".pdf")


fOut = ROOT.TFile("histos/IsoEfficiency/histos_IsoNeuEfficiencySimFit_"+CHANNEL+"_"+ETA+".root","RECREATE")
fOut.cd()
